 ...
 * @param next
 */
export const questionAnswers = async (req, res, next) => {
  try {
    ...
    if (subpanelId) {
      conditions.id = subpanelId;
    }

    let subpanelUserJoinedConditions = { status: "active", is_joined: "y" };
    if (userId) {
      subpanelUserJoinedConditions.created_by = userId;
    }

    const includeQuery = [
      {
        model: model.db.subpanelQuestion,
        attributes: ["id", "subpanel_question", "question_type"],
        where: { status: "active" },
        include: [
          {
            model: model.db.subpanelQuestionOption,
            attributes: ["id", "option_title"],
            where: { status: "active" },
            required: false,
      ...
      },
      {
        ...
        attributes: ["id"],
        where: subpanelUserJoinedConditions,
        required: false,
      },
    ];
    ...
    const selectAttributes = [
      "id",
      "subpanel_title",
      [Sequelize.fn("DATE_FORMAT", Sequelize.col("subpanel.created_at"), "%Y-%m-%d"), "created_at"],
    ];
