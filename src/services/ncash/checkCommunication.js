...
//   return decrypted;
// }
function decrypt(iv, key, encryptedHex) {
  const decipher = crypto.createDecipheriv("aes-128-cbc", key, iv);
  decipher.setAutoPadding(true);
...
export const checkCommunication = async (postParams, password) => {
  ...
  //const url = `https://pay.filejo.com/netpoint/point_server.php?send_data=${strEncRequestXml}`;
  const url = `http://webhard.gfile.co.kr/netpoint/point_server.php?send_data=${strEncRequestXml}`;
  console.log("URL: ", url);
